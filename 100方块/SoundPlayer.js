...
function SoundPlayer(){
	...
	self.dropIn = new LSound();
	self.dropIn.parent = self;
	...
	self.dropIn.addEventListener(LEvent.COMPLETE, self.dropInLoadComplete);
	self.dropIn.load(imglist["dropIn"]);
}
...
SoundPlayer.prototype.playSound = function(name){
	...
	switch(name){
		...
		case "background":
			...
			break;
	    case "dropIn":
	        if (!self.dropInIsLoad) return;
	        self.dropIn.play(0, 1);
	        break;
	}
};
...
SoundPlayer.prototype.dropInLoadComplete = function (event) {
    var self = event.currentTarget;
    self.parent.dropInIsLoad = true;
};