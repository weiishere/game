...
(function (exp) {
    ...
    var initPages = {
        indexPage: function () {
            ...
            gameMainPanel = createSprite({ x: 35, y: 200 });
            bgBlocks = new Array();
            for (var i = 0; i < item_x; i++) {
                var _y = 55 * i;
                bgBlocks.push(new Array());
                for (var j = 0; j < item_y; j++) {
                    bgBlocks[i][j] = { sign: 0, color: "#E2E2E2" };
                    var _x = 55 * j;
                    ...
                    //gameMainPanel.addChild(createSprite({ image: "fk_de_2", x: _x, y: _y }));
                }
            }
    ...
    var gameCommon = {
        ...
        init: function () {
            ...
            backLayer.addEventListener(LMouseEvent.MOUSE_OUT, function (e) {
                if (activeBlock) {
                    LTweenLite.to(activeBlock, 0.02, { scaleX: 0.5, scaleY: 0.5, x: activeBlock.position.x, y: activeBlock.position.y, ease: LEasing.Sine.easeOut, })
                }
            });
            ...
            for (var i = 0, l = arr.length; i < l; i++) {
                (function (arg) {
                    ...
                    _shape.addEventListener(LMouseEvent.MOUSE_DOWN, function (e) {
                        ...
                        _self.state = 2;
                        activeBlock = _self;
                        LTweenLite.to(_self, 0.02, { scaleX: 1, scaleY: 1 });
                    });
                })(i);
            }
            //初始化方块组
        ...
        addWaitBlocks: function () {
            ...
            LTweenLite.to(_block1, 0.03, { x: b_x1, alpha: 1, onComplete: function (e) { _block1.position = { x: _block1.x, y: _block1.y }; } });
            LTweenLite.to(_block2, 0.03, { x: b_x2, alpha: 1, onComplete: function (e) { _block2.position = { x: _block2.x, y: _block2.y }; } });
            LTweenLite.to(_block3, 0.03, { x: b_x3, alpha: 1, onComplete: function (e) { _block3.position = { x: _block3.x, y: _block3.y }; } });
        ...
        isGameOver: function () {
            ...
            backLayer.addChild(initPages.gameOver());
            //alert("game over");
            return false;  
        },
    ...
    var sharpStorage = (function () {
        ...
        sharpTypes.push({ name: "", color: "#BFD09B", data: [[1]] });
    ...
    Number.prototype.changeSpeed = 0.03;
    ...
    init(5, "canvas", 640, 960, function () {
        main();
    });
})(window);